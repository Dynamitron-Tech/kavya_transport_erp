...
test.describe('Market Trips', () => {
  test.beforeEach(async ({ page }) => {
    await gotoAndReady(page, '/market-trips');
  });

  test('page heading is visible after load', async ({ page }) => {
    await expectHeading(page, /market trip/i);
  });
  ...
  test('primary create action works end to end', async ({ page }) => {
    await runPrimaryCreateFlow(page, {
      openButton: /add market trip|new market trip|create market trip/i,
      prefix: 'Market Trips E2E',
    });
