...
test.describe('Driver Leaderboard', () => {
  test.beforeEach(async ({ page }) => {
    await gotoAndReady(page, '/fleet/driver-leaderboard');
  });

  test('page heading is visible after load', async ({ page }) => {
    await expectHeading(page, /driver|leaderboard/i);
  });
  ...
  test('primary create action works end to end', async ({ page }) => {
    await runPrimaryCreateFlow(page, {
      openButton: /refresh|new|create/i,
      prefix: 'Driver Leaderboard E2E',
    });
