...
test.describe('Lorry Receipts', () => {
  test.beforeEach(async ({ page }) => {
    await gotoAndReady(page, '/lr');
  });

  test('page heading is visible after load', async ({ page }) => {
    await expectHeading(page, /lorry receipt|lr/i);
  });
  ...
  test('list loads with data', async ({ page }) => {
    await ensureListHasData(page, TEST.lrNumber);
  });

  test('primary create action works end to end', async ({ page }) => {
    await runPrimaryCreateFlow(page, {
      openButton: /create lr|new lr|add lr/i,
      prefix: 'Lorry Receipts E2E',
    });
