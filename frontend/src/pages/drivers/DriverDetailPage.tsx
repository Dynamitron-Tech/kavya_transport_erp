...
function BehaviourTab({ driverId }: { driverId: number }) {
  ...
  const { metrics, events, speed_distribution, daily_trends } = data;
...
function PerformanceTab({ driverId }: { driverId: number }) {
  ...
  const gradeColor = data.grade === 'A+' || data.grade === 'A' ? '#22c55e' : data.grade.startsWith('B') ? '#3b82f6' : '#f59e0b';
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="card flex flex-col items-center justify-center">
          <ScoreRing score={Math.round(data.overall_score)} label="Overall" size={120} color={gradeColor} />
          <div className="mt-2 flex items-center gap-2">
            <span className="text-2xl font-extrabold" style={{ color: gradeColor }}>{data.grade}</span>
  ...
  return (
    <div className="space-y-5">
      ...
      <nav className="breadcrumb">
        ...
        <ChevronRight size={14} className="breadcrumb-separator" />
        <span className="text-gray-900 font-medium">{driver.full_name}</span>
      </nav>
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="w-14 h-14 rounded-full bg-primary-100 flex items-center justify-center flex-shrink-0">
          {driver.photo_url ? (
            ...
            <span className="text-primary-700 font-bold text-xl">{driver.full_name.charAt(0)}</span>
          )}
        </div>
