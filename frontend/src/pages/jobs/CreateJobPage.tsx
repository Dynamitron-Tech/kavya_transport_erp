...
// 4-Step Wizard with validation, GST calculation,
// Draft/Submit workflow, auto-fill, distance calculation
// ============================================================

import { useState, useEffect, useMemo, useCallback } from 'react';
import { useNavigate, useParams, useSearchParams, Link } from 'react-router-dom';
import { useQuery, useMutation } from '@tanstack/react-query';
import { jobService, vehicleService, driverService } from '@/services/dataService';
...
import {
  ...
  AlertTriangle, ChevronDown, Upload, Zap, RotateCw, MapPinned, User
} from 'lucide-react';
...
const TOTAL_STEPS = 4;
const STEP_LABELS = ['Job & Client', 'Route & Cargo', 'Pricing & Schedule', 'Review & Submit'];
...
export default function CreateJobPage() {
  ...
  const { data: vehiclesListData } = useQuery({
    ...
    queryFn: () => vehicleService.list({ page: 1, page_size: 200 }),
  });

  const { data: driversListData } = useQuery({
    queryKey: ['drivers-list'],
    queryFn: () => driverService.list({ page: 1, page_size: 200 }),
  });
  ...
  // Pre-fill client from URL search params (?client_id=...)
  useEffect(() => {
    const clientIdParam = searchParams.get('client_id');
    if (clientIdParam && !isEdit && !form.client_id) {
      const cid = Number(clientIdParam);
      if (cid > 0) {
        api.get(`/jobs/lookup/clients`, { params: { search: '' } }).then((data: any) => {
  ...
  const vehiclesList = useMemo(() => {
    const raw = vehiclesListData;
    if (!raw) return [];
    const arr = Array.isArray(raw) ? raw : raw?.items || (raw as any)?.data || [];
    return arr;
  }, [vehiclesListData]);

  const driversList = useMemo(() => {
    const raw = driversListData;
    if (!raw) return [];
    const arr = Array.isArray(raw) ? raw : raw?.items || (raw as any)?.data || [];
    return arr;
  }, [driversListData]);
  ...
  // ── Client selection with origin auto-fill ──
  const handleSelectClient = (client: ClientOption) => {
    ...
    // Auto-fill origin from client address
    if (client.address_line1 || client.city || client.state || client.pincode) {
      if (client.address_line1) updateField('origin_address', client.address_line1);
      if (client.city) updateField('origin_city', client.city);
      if (client.state) updateField('origin_state', client.state);
      if (client.pincode) updateField('origin_pincode', client.pincode);
  ...
  // ── Calculate distance using OSRM (free) ──
  const calculateDistance = useCallback(async () => {
    const origin = form.origin_city.trim();
    const destination = form.destination_city.trim();
    if (!origin || !destination) return;
    setIsCalculatingDistance(true);
    try {
      const data: any = await api.get('/maps/distance', {
        params: { origin, destination },
      });
      const dist = data?.distance_km ?? data?.data?.distance_km;
      if (dist && dist > 0) {
        updateField('estimated_distance_km', Math.round(dist * 10) / 10);
        setDistanceAutoFilled(true);
      }
    } catch {
  ...
  }, [form.origin_city, form.destination_city, updateField]);

  // Auto-calculate distance when origin and destination cities change
  useEffect(() => {
    if (form.origin_city && form.destination_city && !form.estimated_distance_km) {
  ...
  // ── Auto-assign vehicle/driver after job creation ──
  const autoAssign = useCallback(async (jobId: number) => {
    try {
      if (form.vehicle_id && form.vehicle_id > 0) {
        await api.put(`/jobs/${jobId}/assign`, { vehicle_id: form.vehicle_id, driver_id: form.driver_id || undefined });
      }
    } catch {
  ...
  return (
    <div className="min-h-screen bg-gray-50/50">
      ...
      >
            ...
            return (
              <div key={step} className="flex items-center gap-2 flex-1">
                ...
                {idx < STEP_LABELS.length - 1 && (
                  <div className={`w-6 h-0.5 flex-shrink-0 ${isCompleted ? 'bg-emerald-300' : 'bg-gray-200'}`} />
                )}
              </div>
            );
          })}
        ...
        {/* ══════════════════════════════════════ */}
        {/* STEP 1: Job Details & Client Info     */}
        {/* ══════════════════════════════════════ */}
        {currentStep === 1 && (
          <>
            ...
            <SectionCard title="Job Details" subtitle="Basic job information and classification" icon={<FileText size={18} />}>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
                ...
                <FormField label="Client Ref / PO Number">
                  <TextInput
                  ...
                  <div className="flex gap-2">
                    {CONTRACT_TYPES.map((ct) => (
                      <button
                        ...
                        className={`flex-1 px-3 py-2 text-xs font-semibold rounded-lg border transition-all text-center ${
                  ...
                  <div className="flex gap-2">
                    {PRIORITIES.map((p) => (
                      <button
                        ...
                        className={`flex-1 px-2 py-2 text-xs font-semibold rounded-lg transition-all text-center ${
            ...
            <SectionCard title="Client Information" subtitle="Select client for this job" icon={<Building2 size={18} />}>
                ...
                <div className="relative">
                  <FormField label="Client" required error={errors.client_id}>
                    <div className="relative">
                      ...
                      <div className="flex items-center border border-gray-300 rounded-lg bg-white focus-within:border-primary-500 focus-within:ring-1 focus-within:ring-primary-200">
                        <div className="pl-3 text-gray-400"><Search size={16} /></div>
                        <input
                          type="text"
                          value={selectedClient ? `${selectedClient.name} (${selectedClient.code})` : clientSearch}
                          onChange={(e) => {
                      ...
                      {showClientDropdown && clients.length > 0 && (
                        <div className="absolute z-50 mt-1 w-full bg-white rounded-xl border border-gray-200 shadow-xl max-h-60 overflow-y-auto">
                          {clients.map((c) => (
                            <button
                              key={c.id}
                              type="button"
                            ...
                            >
                              <div className="flex items-center justify-between">
                                <div>
                                  ...
                                  <p className="text-xs text-gray-500">{c.code} {c.city ? `• ${c.city}` : ''} {c.gstin ? `• ${c.gstin}` : ''}</p>
                                </div>
                                {c.outstanding !== undefined && c.outstanding !== null && Number(c.outstanding) > 0 && (
                  ...
                  <div className="bg-gray-50 rounded-xl border border-gray-200 p-5">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm font-medium text-gray-900">{selectedClient.code}</p>
                      </div>
                      {selectedClient.gstin && (
                        <div>
                          <p className="text-xs text-gray-400">GSTIN</p>
                          <p className="text-sm font-mono text-gray-900">{selectedClient.gstin}</p>
                        </div>
                      )}
                      {selectedClient.city && (
                        <div>
                      ...
                      {selectedClient.credit_limit !== undefined && selectedClient.credit_limit !== null && (
                        <div>
                          <p className="text-xs text-gray-400">Credit Limit</p>
                          <p className="text-sm text-gray-900">₹{Number(selectedClient.credit_limit).toLocaleString('en-IN')}</p>
                        </div>
                      )}
                      {selectedClient.credit_days !== undefined && selectedClient.credit_days !== null && (
                        <div>
                          <p className="text-xs text-gray-400">Credit Days</p>
                          <p className="text-sm text-gray-900">{selectedClient.credit_days} days</p>
                        </div>
                      )}
                    </div>
                  </div>
          ...
          <>
            ...
            <SectionCard title="Route Details" subtitle="Origin, destination, and distance" icon={<MapPin size={18} />}>
              <div className="space-y-6">
                {/* Quick Route Select */}
                {routes.length > 0 && (
                  <FormField label="Quick Route Select" hint="Select a saved route to auto-fill">
                    <SelectInput
                      value={form.route_id}
                      ...
                      options={routes.map((r) => ({
                        value: r.id,
                        label: `${r.origin_city} → ${r.dest_city} (${r.distance_km} km)`,
                      }))}
                    />
                  </FormField>
                )}
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center gap-2">
                      ...
                      {originAutoFilled && (
                        <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-amber-50 text-amber-700 text-xs font-medium border border-amber-200">
                          <Zap size={10} /> Auto-filled from client
                        </span>
                      )}
                    </div>
                    <FormField label="Address" required error={errors.origin_address}>
                      <TextInput
                        ...
                        onChange={(v) => { updateField('origin_address', v); setOriginAutoFilled(false); }}
                    ...
                    <div className="grid grid-cols-3 gap-3">
                      <FormField label="City" required error={errors.origin_city}>
                        <TextInput
                          ...
                          onChange={(v) => { updateField('origin_city', v); setOriginAutoFilled(false); setDistanceAutoFilled(false); }}
                          placeholder="City"
                    ...
                    <div className="grid grid-cols-3 gap-3">
                      <FormField label="City" required error={errors.destination_city}>
                        <TextInput
                          ...
                          onChange={(v) => { updateField('destination_city', v); setDistanceAutoFilled(false); }}
                          placeholder="City"
                  ...
                  <FormField label="Estimated Distance (km)">
                    <div className="flex items-center gap-2">
                      <TextInput
                        ...
                        onChange={(v) => { updateField('estimated_distance_km', Number(v)); setDistanceAutoFilled(false); }}
                        placeholder="0"
                        suffix="km"
                      />
                      <button
                        ...
                        disabled={isCalculatingDistance || !form.origin_city || !form.destination_city}
                        className="flex items-center gap-1 px-3 py-2.5 text-xs font-medium rounded-lg border border-gray-300 hover:bg-gray-50 disabled:opacity-40 transition-colors whitespace-nowrap"
                      >
                    ...
                    {distanceAutoFilled && form.estimated_distance_km > 0 && (
                      <p className="mt-1 text-xs text-emerald-600 flex items-center gap-1">
                        <MapPinned size={12} /> Distance auto-calculated via OSRM
                      </p>
                    )}
                  </FormField>
                </div>
            ...
            <SectionCard title="Cargo Details" subtitle="Material information and special handling" icon={<Package size={18} />}>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                <FormField label="Material Type" required={form.status !== 'draft'} error={errors.material_type}>
                  <TextInput
                    ...
                    placeholder="e.g., Cement, Steel, Electronics"
                ...
                <FormField label="No. of Packages" error={errors.num_packages}>
                  <TextInput
                    type="number"
                    value={form.num_packages || ''}
                    onChange={(v) => updateField('num_packages', Math.max(0, Number(v)))}
                    placeholder="0"
                    error={!!errors.num_packages}
                  />
                </FormField>
        ...
        {currentStep === 3 && (
          <>
            ...
            <SectionCard title="Vehicle & Driver Assignment" subtitle="Assign a vehicle and driver to this job" icon={<User size={18} />}>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <FormField label="Assign Vehicle" hint="Optional — can be assigned later">
                  <SelectInput
                    value={form.vehicle_id}
                    onChange={(v) => updateField('vehicle_id', Number(v))}
                    placeholder="— Select vehicle —"
                    options={vehiclesList.map((v: any) => ({
                      value: v.id,
                      label: `${v.registration_number || v.reg_number || v.vehicle_number || `Vehicle #${v.id}`}${v.vehicle_type ? ` (${v.vehicle_type})` : ''}`,
                    }))}
                  />
                </FormField>

                <FormField label="Assign Driver" hint="Optional — can be assigned later">
                  <SelectInput
                    value={form.driver_id}
                    onChange={(v) => updateField('driver_id', Number(v))}
                    placeholder="— Select driver —"
                    options={driversList.map((d: any) => ({
                      value: d.id,
                      label: `${d.name || d.full_name || `Driver #${d.id}`}${d.license_number ? ` (${d.license_number})` : ''}`,
                    }))}
                  />
                </FormField>
              </div>
        ...
        {/* ══════════════════════════════════════ */}
        {currentStep === 4 && (
          <SectionCard title="Review & Submit" subtitle="Review all details before saving" icon={<CheckCircle2 size={18} />}>
            <div className="space-y-6">
              {/* Job & Client */}
              <div>
                <h4 className="text-sm font-bold text-gray-700 mb-3 flex items-center gap-2">
                  <FileText size={14} /> Job & Client
                </h4>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 bg-gray-50 rounded-lg p-4">
                  <div>
                    <p className="text-xs text-gray-400">Job Date</p>
                    <p className="text-sm font-medium">{form.job_date}</p>
                  </div>
                  <div>
                    <p className="text-xs text-gray-400">Contract</p>
                    <p className="text-sm font-medium capitalize">{form.contract_type}</p>
                  </div>
                  <div>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4 bg-gray-50 rounded-lg p-4">
                  <div>
                    ...
                    <p className="text-sm font-medium">{form.origin_city}{form.origin_state ? `, ${form.origin_state}` : ''}</p>
                  </div>
                  <div>
                    <p className="text-xs text-gray-400">Destination</p>
                    <p className="text-sm font-medium">{form.destination_city}{form.destination_state ? `, ${form.destination_state}` : ''}</p>
                  </div>
                  <div>
                    <p className="text-xs text-gray-400">Distance</p>
                    <p className="text-sm font-medium">{form.estimated_distance_km ? `${form.estimated_distance_km} km` : '—'}</p>
                  </div>
                </div>
              </div>
              ...
              <div>
                <h4 className="text-sm font-bold text-gray-700 mb-3 flex items-center gap-2">
                  <Package size={14} /> Cargo
                </h4>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 bg-gray-50 rounded-lg p-4">
                  <div>
                    <p className="text-xs text-gray-400">Material</p>
                    <p className="text-sm font-medium">{form.material_type || '—'}</p>
                  </div>
                  <div>
                    <p className="text-xs text-gray-400">Quantity</p>
                    <p className="text-sm font-medium">{form.quantity ? `${form.quantity} ${form.quantity_unit}` : '—'}</p>
                  </div>
                  <div>
                    <p className="text-xs text-gray-400">Declared Value</p>
                    <p className="text-sm font-medium">{form.declared_value ? `₹${Number(form.declared_value).toLocaleString('en-IN')}` : '—'}</p>
                  </div>
                  <div>
                    <p className="text-xs text-gray-400">Hazardous</p>
                    <p className="text-sm font-medium">{form.is_hazardous ? 'Yes' : 'No'}</p>
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 bg-gray-50 rounded-lg p-4">
                  <div>
                    ...
                    <p className="text-sm font-medium">₹{(Number(form.agreed_rate) || 0).toLocaleString('en-IN')} ({form.rate_type})</p>
                  </div>
                  <div>
                    <p className="text-xs text-gray-400">GST</p>
                    <p className="text-sm font-medium">{form.gst_percentage}%</p>
                  </div>
                  <div>
                    <p className="text-xs text-gray-400">Total Amount</p>
                    <p className="text-sm font-bold text-emerald-700">₹{(pricing.total ?? 0).toLocaleString('en-IN')}</p>
                  </div>
                </div>
              </div>
              ...
              <div>
                <h4 className="text-sm font-bold text-gray-700 mb-3 flex items-center gap-2">
                  <Truck size={14} /> Vehicle & Schedule
                </h4>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 bg-gray-50 rounded-lg p-4">
                  <div>
                    <p className="text-xs text-gray-400">Vehicle Type</p>
                    <p className="text-sm font-medium">{form.vehicle_type_required || '—'}</p>
                  </div>
                  <div>
                    <p className="text-xs text-gray-400">Pickup Date</p>
                    <p className="text-sm font-medium">{form.pickup_date ? new Date(form.pickup_date).toLocaleString('en-IN') : '—'}</p>
                  </div>
                  <div>
                    <p className="text-xs text-gray-400">Assigned Vehicle</p>
                    <p className="text-sm font-medium">
                      {form.vehicle_id ? vehiclesList.find((v: any) => v.id === form.vehicle_id)?.registration_number || `#${form.vehicle_id}` : '—'}
                    </p>
                  </div>
                  <div>
                    <p className="text-xs text-gray-400">Assigned Driver</p>
                    <p className="text-sm font-medium">
                      {form.driver_id ? driversList.find((d: any) => d.id === form.driver_id)?.name || `#${form.driver_id}` : '—'}
                    </p>
                  </div>
                </div>
              </div>
      ...
      <div className="sticky bottom-0 z-30 bg-white/95 backdrop-blur-sm border-t border-gray-200 shadow-2xl -mx-4 sm:-mx-6">
          ...
          <div className="flex items-center gap-3">
            {currentStep < TOTAL_STEPS ? (
              <button
                ...
                className="flex items-center gap-2 px-6 py-2.5 text-sm font-semibold text-white bg-primary-600 rounded-xl hover:bg-primary-700 transition-colors shadow-sm shadow-primary-200"
              >
                Next →
              </button>
            ) : (
