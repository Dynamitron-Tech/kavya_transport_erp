...
class DriverLeave(Base, TimestampMixin):
    ...
    status      = Column(SQLEnum(LeaveStatusEnum, native_enum=False), default=LeaveStatusEnum.PENDING, nullable=False)
...
class DriverAdvanceRequest(Base, TimestampMixin):
    ...
    status      = Column(SQLEnum(AdvanceStatusEnum, native_enum=False), default=AdvanceStatusEnum.PENDING, nullable=False)
    reviewed_by = Column(Integer, ForeignKey("users.id", ondelete="SET NULL"), nullable=True)
    reviewed_at = Column(DateTime, nullable=True)
    review_note = Column(Text, nullable=True)
