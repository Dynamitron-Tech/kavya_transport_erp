...
#
# Design rules:
#  - NEVER hardcode recipient emails ΓÇö always fetch from DB
#  - NEVER send if user email is missing or blank
#  - NEVER block the main request ΓÇö all public methods are fire-and-forget
#  - Catch + log Brevo API errors; retry max 2 times with 3-second back-off
#  - API key is read from settings (env var) ΓÇö never exposed to frontend
...
async def get_user_info(db: AsyncSession, user_id: int) -> tuple[str | None, str | None]:
    ...
    try:
        ...
        if not row:
            logger.warning("[EmailService] User %s not found or inactive ΓÇö skipping email", user_id)
            return None, None

        email, first_name, last_name = row
        if not email or not email.strip():
            logger.warning("[EmailService] User %s has no email address ΓÇö skipping", user_id)
            return None, None
...
async def send_email(to_address: str, subject: str, html_content: str) -> bool:
    ...
    if not api_key or api_key == "YOUR_BREVO_API_KEY_HERE":
        logger.warning("[EmailService] BREVO_API_KEY not configured ΓÇö email to %s not sent", to_address)
        return False
    ...
    # ΓöÇΓöÇ Log to MongoDB (best-effort, never raises) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    try:
...
async def _send_to_user(db: AsyncSession, user_id: int, subject: str, html_content: str) -> None:
    """Fetch user email and send. Swallows all exceptions ΓÇö for background use."""
    try:
...
def notify_job_created(
    ...
    *,
    ...
    Call AFTER db.commit() ΓÇö does NOT use db for writing.
    """
    async def _task():
        try:
