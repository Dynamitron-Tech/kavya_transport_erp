# Email HTML Templates ΓÇö Kavya Transports Transactional Emails
...
# ΓöÇΓöÇ Brand tokens ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
_BRAND_BLUE = "#1E40AF"
_BRAND_LIGHT = "#DBEAFE"
_TEXT_DARK = "#111827"
_TEXT_MID = "#374151"
...
def render_job_created(
    *,
    ...
    subject = f"New Job Assigned ΓÇö {job_number}"
    body = f"""
      ...
      <table cellpadding="0" cellspacing="0" width="100%"
        ...
        <tbody>
          ...
          {_detail_row("Route", f"{origin} ΓåÆ {destination}")}
...
def render_job_status_updated(
    *,
    ...
    subject = f"Job {job_number} Status Updated ΓÇö {new_status.replace('_', ' ').title()}"
    ...
    body = f"""
      ...
      <table cellpadding="0" cellspacing="0" width="100%"
        ...
        <tbody>
          ...
          {_detail_row("Remarks", remarks or "ΓÇö")}
        </tbody>
      </table>
    """
    return subject, _wrap(subject, body)
...
def render_trip_status_updated(
    *,
    ...
    subject = f"Trip {trip_number} ΓÇö {new_status.replace('_', ' ').title()}"
    status_color = _STATUS_COLORS.get(new_status.lower(), _BRAND_BLUE)
    body = f"""
      <h2 style="margin:0 0 6px;color:{_BRAND_BLUE};font-size:18px;">
        Trip Status Update
      ...
      <table cellpadding="0" cellspacing="0" width="100%"
        ...
        <tbody>
          ...
          {_detail_row("Route", f"{origin} ΓåÆ {destination}" if origin else "ΓÇö")}
          {_detail_row("Vehicle", vehicle_number or "ΓÇö")}
          {_detail_row("Driver", driver_name or "ΓÇö")}
          {_detail_row("New Status", new_status.replace("_"," ").title())}
          {_detail_row("Updated By", updated_by)}
          {_detail_row("Remarks", remarks or "ΓÇö")}
        </tbody>
...
def render_invoice_created(
    *,
    ...
    subject = f"Invoice {invoice_number} Generated ΓÇö Γé╣{total_amount}"
    body = f"""
      <h2 style="margin:0 0 6px;color:{_BRAND_BLUE};font-size:18px;">
        Invoice Generated
      </h2>
      ...
      <table cellpadding="0" cellspacing="0" width="100%"
        ...
        <tbody>
          ...
          {_detail_row("Line Items", str(line_items_count) if line_items_count else "ΓÇö")}
...
def render_invoice_paid(
    *,
    ...
    subject = f"Invoice {invoice_number} Marked Paid ΓÇö Γé╣{total_amount}"
    body = f"""
      <h2 style="margin:0 0 6px;color:#059669;font-size:18px;">
        Invoice Marked as Paid Γ£ô
      </h2>
      <p style="margin:0 0 20px;color:{_TEXT_MID};font-size:13px;">
      ...
      <table cellpadding="0" cellspacing="0" width="100%"
        ...
        <tbody>
          ...
          {_detail_row("Proof File", proof_filename or "ΓÇö")}
        </tbody>
      </table>
      <p style="color:{_TEXT_MID};font-size:12px;margin:0;">
...
def render_report_generated(
    *,
    ...
    subject = f"Report Ready ΓÇö {report_title}"
    download_section = ""
    if download_url:
        download_section = f"""
    ...
    body = f"""
      ...
      <table cellpadding="0" cellspacing="0" width="100%"
        ...
        <tbody>
          ...
          {_detail_row("Period", period or "ΓÇö")}
