...
API Spec:
  ...
  Format: JSON ΓÇö { "success": bool, "vehicles": [...] }

Vehicle packet fields:
  id   ΓÇö KTT internal device ID
  vno  ΓÇö Vehicle registration number (no dashes, e.g. "TN72CB7731")
  lat  ΓÇö Latitude (float)
  lon  ΓÇö Longitude (float)
  spd  ΓÇö Speed (km/h)
  crs  ΓÇö Course/heading (degrees, 0ΓÇô360)
  ign  ΓÇö Ignition: 0 = OFF, 1 = ON
  time ΓÇö Timestamp string "YYYY-MM-DD HH:MM:SS" (IST)
...
# ΓöÇΓöÇ Registration number normalisation ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
_RE_NON_ALNUM = re.compile(r"[^A-Z0-9]", re.IGNORECASE)


def normalise_reg_number(raw: str) -> str:
    """Strip dashes/spaces and uppercase: 'TN-72-CB-7731' ΓåÆ 'TN72CB7731'."""
    return _RE_NON_ALNUM.sub("", raw).upper()


# ΓöÇΓöÇ KTT API client ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
async def fetch_ktt_positions() -> list[dict]:
    ...
    if not token:
        logger.warning("[KTT] KTT_ACCESS_TOKEN not configured ΓÇö skipping poll")
        return []
    ...
    try:
        ...
        logger.error("[KTT] HTTP %s ΓÇö %s", exc.response.status_code, exc.response.text[:300])
        return []
...
def _parse_ktt_packet(pkt: dict) -> dict:
    ...
    dt_raw = pkt.get("time", "")
    try:
        # KTT API returns IST (UTC+5:30) ΓÇö convert to UTC so last_gps_at is stored correctly
...
# ΓöÇΓöÇ Ingest pipeline ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    async with AsyncSessionLocal() as db:
        ...
        for pos in positions:
            try:
                ...
                if not vehicle_id:
                    logger.debug(
                        "[KTT] Vehicle %s not in DB ΓÇö skipping",
                        pos["registration_number"],
                    )
                    skipped += 1
                    continue

                # ΓöÇΓöÇ 1. Update PostgreSQL Vehicle row ΓöÇΓöÇ
                update_values: dict = {
                ...
                # ΓöÇΓöÇ 2. Store in MongoDB ΓöÇΓöÇ
                await _store_telemetry_mongo(vehicle_id, pos)

                # ΓöÇΓöÇ 3. Broadcast via WebSocket ΓöÇΓöÇ
...
# ΓöÇΓöÇ High-level poll-and-ingest ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ

async def poll_and_ingest() -> dict:
    """
    Single poll cycle: fetch from KTT API ΓåÆ ingest into DB.
