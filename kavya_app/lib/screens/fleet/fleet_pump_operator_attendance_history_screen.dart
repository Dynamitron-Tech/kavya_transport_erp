...
final _pumpAttendanceProvider = FutureProvider.autoDispose
    .family<Map<String, dynamic>, _PumpAttendanceParams>((ref, params) async {
  final api = ref.read(apiServiceProvider);
  final res = await api.get(
    '/users/${params.userId}/attendance',
...
class _PumpAttendanceParams {
  final int userId;
  final String month;
  const _PumpAttendanceParams(this.userId, this.month);
  @override
  bool operator ==(Object o) =>
      o is _PumpAttendanceParams && o.userId == userId && o.month == month;
  @override
...
class FleetPumpOperatorAttendanceHistoryScreen extends ConsumerStatefulWidget {
  final int userId;
  final String operatorName;

  const FleetPumpOperatorAttendanceHistoryScreen({
    super.key,
  ...
  @override
  ConsumerState<FleetPumpOperatorAttendanceHistoryScreen> createState() =>
      _State();
}

class _State
    extends ConsumerState<FleetPumpOperatorAttendanceHistoryScreen> {
  ...
  Widget build(BuildContext context) {
    final params = _PumpAttendanceParams(widget.userId, _monthKey);
    final asyncData = ref.watch(_pumpAttendanceProvider(params));
          ...
          children: [
            Text('Attendance History',
                style: KTTextStyles.h2
                    .copyWith(color: KTColors.textHeading, fontSize: 16)),
            Text(widget.operatorName,
                style:
                    KTTextStyles.bodySmall.copyWith(color: KTColors.textMuted)),
          ],
        ),
      ),
      body: Column(
              ...
              error: (e, _) => Center(
                child: Column(
                  ...
                  children: [
                    ...
                    TextButton(
                      onPressed: () =>
                          ref.invalidate(_pumpAttendanceProvider(params)),
              ...
              children: [
                ...
                if (isPresent && checkIn != null) ...[
                  Icon(Icons.login_rounded,
                      size: 15, color: statusColor.withOpacity(0.8)),
...
class _AttendanceBadge extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Text('$pct%',
          style: TextStyle(
              fontSize: 13, fontWeight: FontWeight.w800, color: _color)),
    );
  }
}
